"""
ПРИВЕТСТВЕННЫЕ СООБЩЕНИЯ - ИСПРАВЛЕННАЯ ВЕРСИЯ
...
async def cmd_setwelcome(update, context):
    ...
    try:
        ...
        print(f"❌ Ошибка в cmd_setwelcome: {e}")
        traceback.print_exc()
...
async def cmd_showwelcome(update, context):
    ...
    try:
        ...
        await update.message.reply_text(text, parse_mode=ParseMode.HTML)
        print("✅ Настройки показаны")
        
    except Exception as e:
        print(f"❌ Ошибка в cmd_showwelcome: {e}")
        traceback.print_exc()
        await update.message.reply_text(f"❌ Ошибка: {str(e)[:100]}")

async def cmd_welcomereset(update, context):
    """Сбросить к стандартному тексту"""
    print("\n🔥 ВЫПОЛНЕНИЕ /welcomereset")
    ...
    try:
        ...
        print(f"❌ Ошибка в cmd_welcomereset: {e}")
        traceback.print_exc()
        await update.message.reply_text(f"❌ Ошибка: {str(e)[:100]}")
...
async def handle_new_member(update, context):
    ...
    try:
        ...
        for new_member in update.message.new_chat_members:
            ...
            print(f"👋 Приветствие отправлено для {new_member.first_name}")
            
    except Exception as e:
        print(f"❌ Ошибка в handle_new_member: {e}")

def register(app):
    print("📝 Регистрация команд welcome.py...")
    ...
    print("✅ welcome.py зарегистрирован")