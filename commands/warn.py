...
async def cmd_warn_list(update, context):
    ...
    warnings_list = []
    for user_id_int, user_name, username, warning_count in users:
        # Проверяем ранг
        rank = get_user_rank_db(user_id_int)
        if rank in ['owner', 'curator']:
            continue
        
        clickable = get_clickable_name(user_id_int, user_name, username)
        max_w = get_user_max_warnings(user_id_int)
...
def register(app):
    ...
    async def cmd_remove_warn_full(update, context):
        return await cmd_remove_warn(update, context)
    
    app.add_handler(MessageHandler(
        filters.COMMAND & filters.Regex(r'^!снять выговор\b'),
        cmd_remove_warn_full
    ))